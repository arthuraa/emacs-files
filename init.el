...
(use-package lilypond-mode
  ; lilypond-words.el is generated by the LilyPond build rather than checked
  ; in, so :pre-build runs the generator (see elisp/GNUmakefile upstream) and
  ; :files ships the result.  Without it, lilypond-mode.el calls (sit-for 5)
  ; at load time to nag about the missing file, which costs 5s of startup.
  :straight (:host github :repo "emacsmirror/lilypond"
             :files ("elisp/*.el" "out/lilypond-words.el")
             :pre-build (("mkdir" "-p" "out")
                         ("python3" "scripts/build/lilypond-words.py"
                          "--el" "--dir=out")))
  ; Load lazily anyway; nothing here is needed until a .ly file is opened.
  :defer t
  :init
...
(use-package lean4-mode
  :straight (lean4-mode
             ...
             :rev :last-release)
  ; Deferring this also keeps lsp-mode out of startup.
  :defer t)
...
; Deferring this also keeps tramp out of startup.
